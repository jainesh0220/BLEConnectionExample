...
//		final boolean filerNearbyOnly = isNearbyFilterEnabled();
        ...
        mDevicesLiveData = new DevicesLiveData(false, false);
    ...
    public boolean isUuidFilterEnabled() {
        return mPreferences.getBoolean(PREFS_FILTER_UUID_REQUIRED, false);
    }
     ...
     * {@link } will try to start scanning.
    ...
    public void stopScan() {
        if (mScannerStateLiveData.isBluetoothEnabled() && mScannerStateLiveData.isScanning()) {
            final BluetoothLeScannerCompat scanner = BluetoothLeScannerCompat.getScanner();
